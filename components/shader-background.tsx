...
// Colours, low → high: #101010, #F5F5F5, #B0B0B0, #3A3A3A (8 slots, 4 used).
const COLORS = new Float32Array(
  [0x10, 0xf5, 0xb0, 0x3a].flatMap((c) => [c / 255, c / 255, c / 255]).concat(
    new Array(12).fill(0)
  )
)

const TIME_SCALE = 0.59
const MAX_DPR = 2
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const uScene = uniform("u_scene")

    gl.uniform3fv(uniform("u_colors[0]"), COLORS)
    gl.uniform4f(uniform("u_shape"), 1.5, 0.48, 0.5, 0.0)
    gl.uniform4f(uniform("u_surface"), 2.4, 0.92, -0.5, 1.0)
    gl.uniform4f(uniform("u_finish"), 3.04, 0.61, 0.016, 0.18)
    gl.uniform4f(uniform("u_transform"), 7.0, 0.0, 0.32, 0.0)
    gl.uniform4f(uniform("u_space"), 0.0, 0.0, 0.0, 0.0)
    gl.uniform4f(uniform("u_cursor"), 0.0, 2.0, 0.65, 0.3)
